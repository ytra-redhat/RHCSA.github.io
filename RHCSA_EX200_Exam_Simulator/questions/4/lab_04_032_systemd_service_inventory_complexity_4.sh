...
IS_LAB=true
LAB_ID="ch04_032_systemd_service_inventory_complexity_4"
...
TASK_1_QUESTION="Write the standard output of \`systemctl --failed --no-pager\` to /tmp/exam/ch04_32_failed.txt."
...
TASK_2_QUESTION="Write the standard output of \`systemctl list-unit-files --type=service --state=enabled --no-pager\` to /tmp/exam/ch04_32_enabled.txt."
...
prepare_lab() {
  mkdir -p /tmp/exam
  rm -rf /tmp/exam/ch04_32* /tmp/rhcsa_4_32 /var/tmp/rhcsa_4_32.img
}

_check_task_1_live() {
  [[ -f /tmp/exam/ch04_32_failed.txt ]]
}

_check_task_2_live() {
  [[ -s /tmp/exam/ch04_32_enabled.txt ]]
}
