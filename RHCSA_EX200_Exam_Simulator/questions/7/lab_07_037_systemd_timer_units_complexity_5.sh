...
TASK_1_HINT="Suggested command: printf '[Service]\\nType=oneshot\\nExecStart=/usr/bin/true\\n' > /tmp/exam/ch07_37.service. Explanation: > overwrites the destination with standard output. printf writes deterministic text, including the requested line breaks."
...
TASK_2_HINT="Suggested command: printf '[Timer]\\nOnCalendar=daily\\n[Install]\\nWantedBy=timers.target\\n' > /tmp/exam/ch07_37.timer. Explanation: > overwrites the destination with standard output. printf writes deterministic text, including the requested line breaks."
