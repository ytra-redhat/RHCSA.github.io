...
TASK_1_QUESTION="Create or overwrite /tmp/exam/ch11_22_registries.conf with the following exact line(s): \"registry.example.com\"."
TASK_1_HINT="Suggested command: printf 'registry.example.com\\n' > /tmp/exam/ch11_22_registries.conf. Explanation: > overwrites the destination with standard output. printf writes deterministic text, including the requested line breaks."
TASK_1_COMMAND_1="printf 'registry.example.com\\n' > /tmp/exam/ch11_22_registries.conf"

TASK_2_QUESTION="Create or overwrite /tmp/exam/ch11_22_notes.txt with the following exact line(s): \"search\"; \"insecure\"; \"blocked\"."
TASK_2_HINT="Suggested command: printf 'search\\ninsecure\\nblocked\\n' > /tmp/exam/ch11_22_notes.txt. Explanation: > overwrites the destination with standard output. printf writes deterministic text, including the requested line breaks."
