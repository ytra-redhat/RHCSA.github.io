...
TASK_1_HINT="Suggested command: install -d -m 775 /tmp/exam/ch11_23_volume. Explanation: install -d creates directories while -m applies the requested mode atomically."
...
TASK_2_HINT="Suggested command: printf 'podman run -d -p 8080:80 -v /tmp/exam/ch11_23_volume:/data:Z IMAGE\\n' > /tmp/exam/ch11_23_run.txt. Explanation: > overwrites the destination with standard output. printf writes deterministic text, including the requested line breaks."
