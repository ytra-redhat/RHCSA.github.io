...
TASK_1_QUESTION="Run firewall-cmd --zone=public --list-services and write both standard output and standard error to /tmp/exam/ch10_20_runtime.txt. Overwrite the destination if it already exists. The task is complete when the destination file exists and contains the requested command output."
TASK_1_HINT="Suggested command: firewall-cmd --zone=public --list-services > /tmp/exam/ch10_20_runtime.txt 2>&1. Explanation: 2>&1 merges standard error into the standard-output stream before it is written. > overwrites the destination with standard output. without --permanent, firewall-cmd queries the current runtime configuration."
TASK_1_COMMAND_1="firewall-cmd --zone=public --list-services > /tmp/exam/ch10_20_runtime.txt 2>&1"

TASK_2_QUESTION="Run firewall-cmd --permanent --zone=public --list-services and write both standard output and standard error to /tmp/exam/ch10_20_permanent.txt. Overwrite the destination if it already exists. The task is complete when the destination file exists and contains the requested command output."
TASK_2_HINT="Suggested command: firewall-cmd --permanent --zone=public --list-services > /tmp/exam/ch10_20_permanent.txt 2>&1. Explanation: 2>&1 merges standard error into the standard-output stream before it is written. > overwrites the destination with standard output. --permanent reads firewalld's saved configuration rather than the current runtime state."
